...
class FlyingObject:
    ...
    def _validate_initial_data(self):
        ...
        if not re.fullmatch(pattern, self.object_id):
            raise ValueError(
                f"Invalid object_id {self.object_id}. It should match the pattern {pattern}"
            )
        ...
        if not re.fullmatch(pattern, self.payload):
            raise ValueError(
                f"Invalid payload {self.payload}. It should match the pattern {pattern}"
            )
    ...
    def update(self, current_time: datetime) -> tuple[Point2D, float, bool]:
        ...
        position, angle = self.path.calculate_position_and_angle(
            self.creation_time, current_time, self.speed
        )
        angle = FlyingObject._normalize_angle(angle)
        # Determine if the object has expired
        if self.arrive_time is None:
            self.arrive_time = self.path.calculate_arrive_time(
                self.creation_time, self.speed
            )
