...
class Generator:
    ...
    def run_simulation(self):
        ...
        with logging_redirect_tqdm(), tqdm(
            ...
            mininterval=3,
            bar_format="{percentage:.1f}%|{bar}| {desc} [{elapsed}<{remaining}",
        ) as pbar:
            while current_time < self.start_time + self.simulation_duration:
                pbar.update(self.simulation_resolution.total_seconds())
                # pbar.set_description(f"Living Objects: {len(self.world.objects)}") # Too slow according to profiler
                # Check if there are any objects to create
                while (
                    len(object_creation_times) > 0
                    and object_creation_times[-1] <= current_time
                ):
                    # Create a new object and add it to the world
                    obj = self._create_object(current_time)
                    destination = self._generate_position(
                        obj.initial_pos, 150 * 1000, 400 * 1000
                    )
                    way_point = self._generate_position(
                        obj.initial_pos, 100 * 1000, 150 * 1000
                    )
    ...
    def _debug_log(self, current_time: datetime):
        ...
        if len(self.world.objects) > 0:
            ...
            logging.debug(
                f"Current time: {current_time} -> {first_object.arrive_time - current_time} {first_object.position.x}"
            )

    def _generate_object_creation_times(
        self, start_time: datetime, simulation_duration: timedelta, num_objects: int
    ) -> list[datetime]:
        """
    ...
    def _generate_position(
        ...
        if start_point and min_distance is not None and max_distance is not None:
            ...
            x = min(
                max(x, self.world.world_size.x_lower.boundary),
                self.world.world_size.x_upper.boundary,
            )
            y = min(
                max(y, self.world.world_size.y_lower.boundary),
                self.world.world_size.y_upper.boundary,
            )
        else:
            # A random position within the world
            x = uniform(
                self.world.world_size.x_lower.boundary,
                self.world.world_size.x_upper.boundary,
            )
            y = uniform(
                self.world.world_size.y_lower.boundary,
                self.world.world_size.y_upper.boundary,
            )
