...
def query_object_trajectory():
    ...
    path_result = db.session.query(FlyingObjectStateOrm).filter(
    ...
    flying_object = db.session.query(FlyingObjectOrm).filter(
        FlyingObjectOrm.object_id == object_id
    ).first()
...
def query_sector_snapshot():
    ...
    objects_result = db.session.query(FlyingObjectStateOrm).filter(
        FlyingObjectStateOrm.sector == sector_id,
        FlyingObjectStateOrm.state_time >= start_time,
        FlyingObjectStateOrm.state_time <= end_time
    ).order_by(FlyingObjectStateOrm.object_id, FlyingObjectStateOrm.state_time).all()
